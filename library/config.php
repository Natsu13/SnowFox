...
class Config {
		...
		function __construct($root){			
			...
			$root->getContainer()->set('config', $this);
		}
		...
		public function update($name, $value){
			$fln = dibi::query("SELECT * FROM :prefix:settings WHERE `name`=%s", $name)->fetch();
			if($fln == null){
				$data = array(
					"name" 	=> $name,
					"value" => $value
				);
				dibi::query('INSERT INTO :prefix:settings', $data);
			}else{
				dibi::query('UPDATE :prefix:settings SET ', array("value" => $value), "WHERE `name`=%s", $name);
			}
			$this->set($name, $value);
			return $value;
		}
		...
		public function getD($name, $default = "", $group = "root"){
			$q = $this->get_variable($name, ($group == true ? "root": $group));
			if($group === true){
				$u = User::getData($name, NULL);
				if($u != NULL)
					return $u;		
			}		
			if(is_null($q))
		...
		public function get_variable($name, $group = "root", $default = NULL){
			if( $this->is_variable( $name, $group ) ){
		...
		public function jload($text){
			$array = json_decode($text, true);
			return $array;
		}

		public function jsave($array){
		...
		public function load_variables($group = "root", $file = null){		
			$this->variables[$group] = $this->open(_ROOT_DIR . "/config/" . ($file ?? $group) . ".config");
		...
		public function get_group($group = "root"){
			return $this->variables[$group];
		}
		
		public function test($group = "root"){
			//if($this->load_variables()){
				foreach($this->get_group($group) as $k => $v){
					echo $k." : ".$v["Value"]."<br>";
				}
			//}
		}
